// This was lazily copied from metrics, and has a lot more than I need.  I just
// don't want to bother removing the dead code.
...
exports.get = function(req, res) {
    ...
    options.current_semester().then(function(sem) {
        ...
        if (p.is_admin(req)) {
            ...
            begintime.setDate(begintime.getDate()-365);
        ...
        console.log("ERROR: could not render history: " + error.message);
        if (!res.headersSent) {
            res.sendStatus(500);
        }
    });
...
// Not currently routed, but kept in sync with the copy in metrics.js so that
// mounting it later doesn't reintroduce an unauthenticated endpoint.
